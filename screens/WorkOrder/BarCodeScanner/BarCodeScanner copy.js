...
const handleSubmit = ({ qrcode, getQrcode, navigation }) => {
  getQrcode({ variables: { qrcode: qrcode } })
    .then(response => {
...
const BarcodeScanner = props => {
  ...
  const [getQrcode, { loading, data }] = useLazyQuery(
    CHECK_FOR_WORKORDER,
    {
      // onCompleted(data) {
  ...
  const addWithoutQr = () => {
    ...
    var genQr = "000006";
    setQrcode(genQr);
    getQrcode({ variables: { qrcode: "000006" } });
    console.log(data);
    // .then(response => {
    //   const { workorder } = response;
    //   if (!workorder) {
